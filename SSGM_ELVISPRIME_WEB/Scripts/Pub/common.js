        ...
        bind : function(){ // 이벤트 바인딩
			...
			$(window).resize(function () {
				...
				if (!$('.total_menu').hasClass('on')) {
					$('#hamberger.mo.show').removeClass('show');
                }
			})
			//if ($('.total_menu').hasClass('on')) {
			//	if (matchMedia("screen and (min-width: 1025px)").matches) {
...
var layerPopup = function(obj){
	...
	$glayer_zone.hide();
	$("body").addClass("noscroll");   // ★본문스크롤 제거
	$laybtn.fadeIn(200);
	enable();
	...
	$glayer_zone.on("click",".close",function(e){
		...
		$("body").removeClass("noscroll");   // ★본문스크롤 제거
		t_layer.fadeOut(300);
		disable();
	});
...
var layerPopup3 = function (obj, target) {
	var $laybtn = $(obj),
		$glayer_zone = $(".layer_zone");
	$focus = target;
	if ($glayer_zone.length === 0) { return; }
...
function fnMovePage(vId) {
	...
	var windowWidth = $(window).width();
	var vHeaderHeight = $("#header").height() - 50;
...
$(document).on('click', '.cell .int', function () {
	$('.cell').removeClass('on');
	$(this).parents('.cell').addClass('on');
})

$('.cell:first-child input').keydown(function (e) {
	$('.cell').removeClass('on');
	if (e.keyCode === 9) { // Tab 키의 keyCode는 9입니다.
		$('.cell:nth-child(2)').addClass('on');
	}
});

$(document).on('click', '.btn-close', function () {
	$(this).parents('.sch-pop').hide();
})

$(document).on('click', '.close_select', function () {
	$(this).parents('.select_pop').hide();
})

$('html').click(function (e) {
	if ($(e.target).parents('.select_pop').length < 1) {
		$('.select_pop').hide();
	}
});
...
$(document).on('click', '.total_menu.on .lnb > li', function () {
	$(this).find('.sub_depth').slideDown(200);
	$(this).addClass('click');
})

$(document).on('click', '.total_menu.on .lnb > li.click', function () {
	$(this).find('.sub_depth').slideUp(200);
	$(this).removeClass('click');
})
$(document).on('click', '#sub', function () {
	var test = $(this).find('.sub_depth');
	var test2 = $(this).find('a');
	test.css('display', 'none');
	test2.css('background', 'unset');
})
...
$(document).ready(function () {
	$('.sub_depth ul').hover(function () {
		$(this).parents('.sub_depth').parents('li').children('a').css('text-decoration', 'underline');
		$(this).parents('.sub_depth').parents('li').children('a').css('text-underline-position', 'under');
		$(this).parents('.sub_depth').parents('li').children('a').css('font-weight', '700');
	}, function () {
		$('.lnb a').css('text-decoration', '');
		$('.lnb a').css('text-underline-position', '');
		$('.lnb a').css('font-weight', '');
	});
});

$(document).on('keyup', '.input-box input', function () {
	var del = $(this).siblings('.delete-btn');
	if ($(this).val().trim() !== "") {
		del.show();
	} else {
		del.hide();
...
$(document).on("keyup", "#input_POD, #sch_pod_popup", function () {
