...
var _DocType_HBL_CFS = "'CIPL', 'HBL', 'CO', 'CC', 'IP','HDC' ,'DO','ETCH'"; //INV는 따로
var _DocType_Part_CFS = "'CIPL', 'HBL', 'CO','PDC'";  //파트너
////////////////////jquery event///////////////////////
$(function () {
    //로그인 세션 확인
...
$(document).on("click", "a[name='btn_FileList']", function () {
    ...
    objJsonData.HBL_DOC_TYPE = _DocType_HBL_CFS;
    objJsonData.PARTNER_DOC_TYPE = _DocType_Part_CFS;
...
//    fnLayer_Console_Detail(_vCfsNo)
//});
...
//                vHTML += "   	<td class=\"txt\">" + fnSetComma(_fnToNull(vResult[i]["BK_QTY"])) + "</td> ";
//                vHTML += "   	<td class=\"txt\">" + _fnToNull(vResult[i]["BK_QTY_UNIT"]) + "</td> ";
//                vHTML += "   	<td class=\"txt\">" + fnSetComma(_fnToNull(vResult[i]["BK_GRS_WGT"])) + "</td> ";
//                vHTML += "   	<td class=\"txt\">" + fnSetComma(_fnToNull(vResult[i]["BK_MSRMT"])) + "</td> ";
...
//                vHTML += "                                                    <p class='des'>" + fnSetComma(_fnToNull(vResult[i]["BK_QTY"])) + "</p> ";
//                vHTML += "                                                </div> ";
//                vHTML += "                                                <div class='info-txt info-txt--full'>";
...
//                vHTML += "                                                    <p class='des'>" + fnSetComma(_fnToNull(vResult[i]["BK_MSRMT"])) + "</p> ";
//                vHTML += "                                                </div> ";
//                vHTML += "                                            </div> ";
//                vHTML += "          <input type='hidden' name='GR_NO' value='" + _fnToNull(vResult[i]["GR_NO"]) + "'> ";
//                vHTML += "          <input type='hidden' name='BK_NO' value='" + _fnToNull(vResult[i]["BK_NO"]) + "'> ";
...
$(document).on("click", "#PC_Layer_Console_Header tr", function () {
    ...
    fnLayer_Console_Detail(vBkNo, vBkSeq, vGrNo)
});
...
function fnLayer_Console_Detail(vBkNo, vBkSeq, vGrNo) {
    try {
        var objJsonData = new Object();

        objJsonData.BK_NO = _fnToNull(vBkNo);
        objJsonData.BK_SEQ = _fnToNull(vBkSeq);
        objJsonData.GR_NO = _fnToNull(vGrNo);
        objJsonData.USR_TYPE = _fnToNull($("#Session_USR_TYPE").val());
        objJsonData.CUST_CD = _fnToNull($("#Session_CUST_CD").val());

        $.ajax({
            type: "POST",
            url: "/Popup/fnGetLayerConsoleDetail_MYDATA",
            async: false,
            dataType: "json",
            data: { "vJsonData": _fnMakeJson(objJsonData) },
            success: function (result) {
...
function fnMakeLayerConsole(vJsonData) {
    try {
        ...
        if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
            ...
            vHTML += "   <li class=\"item col-sm-6 col-12\"> ";
            vHTML += "   	<div class=\"text-box row\"> ";
            vHTML += "   		<span class=\"tit col-md-4 col-12\">실입고처</span> ";
            vHTML += "   		<span class=\"des col-md col-12\">" + _fnToNull(vResult[0]["ACT_GR_CUST"]) + "</span> ";
            vHTML += "   	</div> ";
            vHTML += "   </li> ";
            vHTML += "   <li class=\"item col-sm-6 col-12\"> ";
            vHTML += "   	<div class=\"text-box row\"> ";
            ...
            vHTML += "   		<span class=\"des col-md col-12\">" + _fnToNull(vResult[0]["RMK2"]) + "</span> ";
            vHTML += "   	</div> ";
            vHTML += "   </li> ";
            vHTML += "   <li class=\"item col-sm-6 col-12\"> ";
            vHTML += "   	<div class=\"text-box row\"> ";
            ...
            vHTML += "   		<span class=\"des col-md col-12\">" + fnSetComma(_fnToNull(vResult[0]["BK_GRS_WGT"])) + "</span> ";
            vHTML += "   	</div> ";
            vHTML += "   </li> ";
            vHTML += "   <li class=\"item col-sm-6 col-12\"> ";
            vHTML += "   	<div class=\"text-box row\"> ";
    ...
    $(this).autocomplete({
        ...
        open: function (event, ui) {
            $(this).autocomplete("widget").css({
                "width": $(".pop__export").width()
            });
        },
        source: function (request, response) {
            var result = _fnGetPortData($("#input_POL").val().toUpperCase());
            if (result != undefined) {
...
$(document).on("keyup", "#input_POD", function () {
    ...
    $(this).autocomplete({
        ...
        source: function (request, response) {
            var result = _fnGetPortData($("#input_POD").val().toUpperCase());
            if (result != undefined) {
                result = JSON.parse(result).Table
                response(
                    $.map(result, function (item) {
    ...
    try {
        ...
        if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
            ...
            $.each(vResult, function (i) {
                ...
                if (_fnToNull(vResult[i]["HBL_NO"]) != "" && _fnToNull(vResult[i]["BKG_NO"]) != "") {
                    vHTML += "<br>" + "(" + _fnToNull(vResult[i]["BKG_NO"]) + ")";
                }
                vHTML += "   	</td> ";
                vHTML += "   	<td class=\"txt\"> ";
                ...
                if (_fnToNull(vResult[i].FILE_CNT) == "0") {
                    ...
                    vHTML += "              <input type='hidden' name='File_BKG_NO' value='" + _fnToNull(vResult[i]["PRM_BKG_NO"]) + "'> ";
                ...
                //비엘 제출 여부
                // 파트너랑 아닌 사람이랑 나뉨
                if (!(_fnToNull($("#Session_USR_TYPE").val()) == "P" && _fnToNull(vResult[i]["EX_IM_TYPE"]) == "I")) {
            ...
            $.each(vResult, function (i) {
                ...
                vHTML += "   	</li> ";
                vHTML += "   	<li class=\"info-txt info-txt--full\"> ";
                vHTML += "   		<p class=\"title\">House B/L<br>(창고부킹번호) </p> ";
                ...
                if ($("#Session_USR_TYPE").val() != "P") {

                    vHTML += "   	<li class=\"info-txt info-txt--full\"> ";
                    if (_fnToNull(vResult[i]["EX_IM_TYPE"]) == "E") {
                        vHTML += "   		<p class=\"title\">Check B/L</p> ";
                    } else {
                    ...
                    if (_fnToNull(vResult[i]["IS_CHKBL_YN"]) == "Y") {
                        ...
                        if (_fnToNull($("#Session_USR_TYPE").val()) != "P") {
                            vHTML += "   		<span class=\"success-btn btn1 icn_state2\" name=\"layer_BLPrintData\"></span> ";
                            vHTML += "          <input type=\"hidden\" name=\"HBL_NO\" value=\"" + _fnToNull(vResult[i]["HBL_NO"]) + "\"> ";
                        }
                        if (_fnToNull(vResult[i]["EX_IM_TYPE"]) == "E") {
                            vHTML += "          <input type=\"hidden\" name=\"DOC_TYPE\" value=\"CHBL\"> ";
