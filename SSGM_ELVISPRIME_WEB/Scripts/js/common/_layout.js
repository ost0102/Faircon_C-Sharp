        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (JSON.parse(result).Result[0]["trxCode"] == "Y") {
                    if (JSON.parse(result).Check[0]["REQ_SVC"] == "SEA") {
        ...
        if (vREQ_SVC == "SEA") {
             try {
                ...
                //frm.action = "https://txweb-vtrack-test.azurewebsites.net/custom/shipment-detail?blNo=" + _objTrkLayer.MBL_NO;
                 frm.action = "https://shipgo.tradlinx.com/custom/shipment-detail?blNo=" + _objTrkLayer.MBL_NO;
                frm.target = "viewer";
                frm.method = "post";
                frm.submit();
...
////////////////////////function///////////////////////
//port 정보 가져오는 함수
function _fnGetWebPortData(vValue) {
    try {
        var rtnJson;
        var objJsonData = new Object();
...
// Warehouse port 정보 가져오는 함수
function _fnGetWebWarePortData(vValue) {
    try {
        var rtnJson;
        var objJsonData = new Object();
