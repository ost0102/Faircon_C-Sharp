...
function fnSetSurchargePort() {
	...
	$.ajax({
		type: "POST",
		url: "/LogisticsInfo/fnSetSurchargePort",
		async: true,
		cache: false,
		dataType: "Json",
...
function fnSetSurchargeCountry(vValue) {
	try {
		...
		$.ajax({
			type: "POST",
			url: "/LogisticsInfo/fnSetSurchargeCountry",
			async: true,
			cache: false,
			dataType: "Json",
			data: { "vJsonData": _fnMakeJson(objJsonData) },
...
function fnSurchargeSearch() {
	...
	try {
		...
		if (SurchargeValidation()) {
			objJsonData.PORT_TYPE = "S";
			objJsonData.COUNTRY_OPTION = $("select[name='select_CountryList']").eq(0).find("option:selected").val(); //국가옵션 체크
			...
			$.ajax({
				type: "POST",
				url: "/LogisticsInfo/fnSearchSurcharge",
				async: true,
				cache: false,
				dataType: "Json",
				data: { "vJsonData": _fnMakeJson(objJsonData) },
